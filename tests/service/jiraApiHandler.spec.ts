...
describe('JIRA API Handler', () => {
    ...
    it('Jira Status API with Valid Ticket', (done) => {
        let mockResponse = {
            expand: 'renderedFields,names,schema,operations,editmeta,changelog,versionedRepresentations,customfield_10267.serviceName',
            id: '16996',
            ...
            fields: {
                ...
                issuetype: {
                    ...
                    iconUrl: 'https://jatahworx.atlassian.net/secure/viewavatar?size=medium&avatarId=10316&avatarType=issuetype',
                    name: 'Sub-task',
                    subtask: true,
                    avatarId: 10316
                },
         ...
         * method name
         */
        let jiraStub = sinon.stub(jira, 'getTicketStatus');
        jiraStub.withArgs('MO-49').resolves(mockResponse);
        jira.getTicketStatus('MO-49').then(response => {
            setTimeout(function () {
                expect(response).to.deep.equals(mockResponse);
                done();
            })
        }).catch(done);
        jiraStub.restore();
    ...
    it('Jira Status API with Invalid Ticket', (done) => {
        let mockResponse = {
            errorMessages: ['Issue does not exist or you do not have permission to see it.'],
            errors: {}
        }
        let jiraStub = sinon.stub(jira, 'getTicketStatus');
        jiraStub.withArgs('Hello').resolves(mockResponse);
        jira.getTicketStatus('Hello').then(response => {
            setTimeout(function () {
                expect(response).to.deep.equals(mockResponse);
                done();
            })
